...
class DataFrameRemote(DataFrame):
    ...
    def data_type(self, expression, internal=False, array_type=None, axis=0, encode=True):
        if str(expression) in self._dtypes:
            return self._dtypes[str(expression)]
        else:
            if str(expression) not in self._dtype_cache:
                self._dtype_cache[str(expression)] = super().data_type(expression, array_type=array_type, axis=axis, encode=encode)
